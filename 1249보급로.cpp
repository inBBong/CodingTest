...
int main(int argc, char** argv)
{
	...
	*/
	for (test_case = 1; test_case <= T; ++test_case)
	{
		...
		cout << "#" << test_case << " " << solution(N, map) << endl;
...
int solution(int N, vector<vector<int>> map)
{
	...
	queue<Point> q;
	q.push(S);
	vector<vector<int>> weightMap(map);
	while (!q.empty())
	{
		Point currentPos=q.front();
		q.pop();
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			int ncnt = currentPos.cnt + map[ny][nx];
			Point p(nx, ny, ncnt);
			if (!visited[ny][nx])
			{
				weightMap[ny][nx] = ncnt;
			}
			else
			{
				if (ncnt < weightMap[ny][nx])
					weightMap[ny][nx] = ncnt;
				else
					continue;
			}
